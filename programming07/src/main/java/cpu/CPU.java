...
public class CPU {
    ...
    public int execInstr(long number) {
        ...
        while (number > 0) {
            // TODO
            String instr = String.valueOf(mmu.read(CPU_State.cs.read() + CPU_State.eip.read(), 40)).trim();
            int opcode = Integer.valueOf(transformer.binaryToInt(instr.substring(0,8)));
            Instruction instruction = InstrFactory.getInstr(opcode);
            totalLen += instruction.exec(instr.substring(8), opcode);
            break;
        }
        return totalLen;
    }
