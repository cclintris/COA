...
public class Cache {	//
	...
	public char[] read(String eip, int len){
		...
		int addr =  Integer.parseInt(t.binaryToInt(eip));
	...
	public int getBlockNO(String addr) {
		Transformer t = new Transformer();
		return Integer.parseInt(t.binaryToInt(addr.substring(0, 22)));
	}
	...
	public void writeCache(char[] data, int lineNO, char[] tag) {
		CacheLine line = cache.get(lineNO);
		line.data = data;
		line.tag = tag;
		line.validBit = true;
		line.visited = 1;
	...
	public char[] tagInLine(int lineNO) {
		return cache.get(lineNO).tag;
	}

	public int getVisited(int lineNO) {
		return cache.get(lineNO).visited;
	}

	public boolean getValidBit(int lineNO) {
		return cache.get(lineNO).validBit;
	}

	public void visitedIncrement(int lineNO) {
		cache.get(lineNO).visited++;
	}

	public Long getTimestamp(int lineNO) {
		return cache.get(lineNO).timeStamp;
	}

	public void timeStampincrement(int lineNO) {
		cache.get(lineNO).timeStamp++;
	}

	public void printLine(int lineNO) {
		CacheLine line = cache.get(lineNO);
		System.out.println("write " + line.data[0] + " lineNO " + lineNO);

		// System.out.println(lineNO);
		// System.out.println(Arrays.toString(line.tag));
		// System.out.println(Arrays.toString(line.data));
	}
}
